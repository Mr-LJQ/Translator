...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 onStorageChange({
   ...
   sentenceConfig: (_, sentenceConfig) => anki.updateAnkiConfig({ sentenceConfig }),
   ankiConnectionURL: (_, ankiConnectionURL) => anki.updateAnkiConfig({ ankiConnectionURL }),
 })
 ...
 //监听所有发生到后端的请求，并进行处理
 onMessage(async ({ command, data }, sendResponse: SendResponse) => {
   let response
   switch (command) {
     ...
     case Command.TranslateInjectText:
       ...
       postFrontend(Command.ShowInjectTranslation, { translatedData: response, point: data.point })
       break
